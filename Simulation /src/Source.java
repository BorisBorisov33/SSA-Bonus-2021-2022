...
public class Source implements CProcess
{
    ...
    public void execute(int type, double tme)
    {
        ...
        arrivalTimes.add(tme);
        // give arrived product to queue
        // if a regular customer then type == 0  otherwise type == 1
        Product p;
        if(type == 0) {
            p = new Product(CustType.valueOf("Regular"));

        }else {
            p = new Product(CustType.valueOf("ServiceDesk"));
        }
    ...
    public static double drawRandomExponential(double mean)
    {
        ...
        double res = random.nextGaussian() * std + mean;
        return res;
    }
}